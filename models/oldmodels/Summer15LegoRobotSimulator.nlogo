...
robots-own [ my-turret bump-sensor-pressed? ]
...
to setup
  ...
  ;set mynet ahninn:create-from-xml xml
  setup-walls  
  setup-goal
  setup-robots  
  reset-ticks
...
to robot-backward [ dist ]
  repeat dist * 10 [
    if not (obstacle-ahead (-0.1 + ROBOT_CENTER_TO_FRONT_BUMPER_LENGTH))
    [
      back 0.1
    ]
  ]
...
to robot-rotate [ angle ]
  left angle ;; TODO: add randomness!
end

;; reports the distance sensor reading (in meters, like the real robots) for the current turret heading 
to-report robot-sense-distance
  debug-flash my-turret blue "Sensing"
  let sensor-result 100 ; 100 patches = 10 m is close enough to "infinity", given this robot course size...
    ...
    debug-flash self green (word (precision sensor-result 2) " m")
  report 10 * sensor-result
end
...
Rectangle -13345367 true false 0 15 330 30
